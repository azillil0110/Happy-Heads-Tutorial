...
<?php
    ...
    foreach ($daysOfWeek as $day) {
        echo "<div class='schedule-cell'>$day</div>";
    }
    echo '</div>';
        ...
        foreach ($groupedSchedules as $day => $schedules) {
            ...
            foreach ($mergedSchedules as $schedule) {
                ...
                if (($scheduleStart >= $slotStart && $scheduleStart < $slotEnd) || ($scheduleEnd > $slotStart && $scheduleEnd <= $slotEnd)) {
                    ...
                    echo "<div class='schedule-cell'>Start: {$schedule['sched_starttime']}<br>End: {$schedule['sched_endtime']}</div>";
                    $found = true;
                    break;
                }
            }
            ...
            if (!$found) {
                echo "<div class='schedule-cell'></div>"; // Empty cell if no schedule matches the time slot
            }
        }
        echo '</div>';
    }
